...
class JsExtraslider extends BaseHelperSlider{
	...
	public static function getList(&$params)
	{
		...
		foreach($items as $key=>$value){
			...
			foreach ($list as $product_info){
				...
				$description = JsExtraslider::_cleanText($product_info->description);
			...
			$value->title = JsExtraslider::truncate($value->name, $limit_title);
			$description_short = JsExtraslider::_cleanText($value->short_description);
			if(empty($description_short)){
				$value->short_description = JsExtraslider::truncate($description, $limit_description);
			}else{
				$value->short_description = JsExtraslider::truncate($description_short, $limit_description);
			}	
			$value->_tax = productTaxInfo($product->getTax()).' '.sprintf(_JSHOP_PLUS_SHIPPING, $product->shippinginfo) ;		
		}
		return $items;	
	}
