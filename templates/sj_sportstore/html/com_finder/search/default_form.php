...
<form id="finder-search" class="form-inline" action="<?php echo JRoute::_($this->query->toURI()); ?>" method="get">
	...
	<fieldset class="word">
		...
		<?php if ($this->escape($this->query->input) != '' || $this->params->get('allow_empty_search')):?>
			<button name="Search" type="submit" class="button btn-primary disabled"><i class="icon-search icon-white"></i><?php echo JText::_('JSEARCH_FILTER_SUBMIT');?></button>
		<?php else: ?>
			<button name="Search" type="" class="button btn-primary disabled"><i class="icon-search icon-white"></i><?php echo JText::_('JSEARCH_FILTER_SUBMIT');?></button>
		<?php endif; ?>
        <?php if ($this->params->get('show_advanced', 1)): ?>
			<a  data-toggle="collapse" href="#advanced-search"class="button" id="advanced-search-toggle"><i class="icon-list icon-white"></i> <?php echo JText::_('COM_FINDER_ADVANCED_SEARCH_TOGGLE'); ?></a>
		<?php endif; ?>
</fieldset>
