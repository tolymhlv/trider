...
JHtml::stylesheet('templates/' . JFactory::getApplication()->getTemplate().'/html/mod_sj_js_reslisting/assets/css/isotope.css');
JHtml::stylesheet('templates/' . JFactory::getApplication()->getTemplate().'/html/mod_sj_js_reslisting/assets/css/sj-reslisting.css');
...
<!--[if gt IE 9]><!--><div id="<?php echo $tag_id; ?>" class="sj-responsive-listing pre-load"><!--<![endif]-->
   ...
   <div class="respl-loading"></div>
	<div class="respl-wrap cf">
		<div class="respl-header-title">
			<ul class="lst-title-cate">
				<?php foreach($list['categories'] as $items){ ?>
				<li class="<?php echo (isset($items->sel))?$items->sel:''; ?>">
					<span class="title-cate"><a href="<?php echo $items->link; ?>" <?php echo JSResponsiveListingHelper::parseTarget($params->get('link_target','_self'))?> title="<?php echo $items->category_title?>" >
							<?php //echo $item->category_title?>
							<span><?php echo ($items->name == JText::_('ALL_LABEL'))?JText::_('ALL_LABEL'):JSResponsiveListingHelper::truncate($items->name, $params->get('tal_max_characters')) ?></span>
					</a></span>
					<span class="cate_short_description"><?php echo $items->short_description;?></span>
				</li>
				<?php }?>
			</ul>		
		</div>
